...
function MeditationTimer() {
  ...
  return (
    <div className="zen-card" style={{ padding: "1.75rem", textAlign: "center" }}>
      <h3 style={{ fontFamily: "'Noto Serif TC','Noto Serif SC',serif", fontSize: "1.2rem", color: "#2c1810", marginBottom: "1.25rem" }}>
        🧘 靜心禪修計時
      </h3>
      <div style={{ position: "relative", width: "140px", height: "140px", margin: "0 auto 1.5rem" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontFamily: "'Noto Serif TC','Noto Serif SC',serif", fontSize: "1.6rem", fontWeight: 600, color: "#2c1810" }}>
            {timeLeft !== null ? fmt(timeLeft) : `${duration}:00`}
          </span>
          <span style={{ fontSize: "0.8rem", color: "#8a5a2f", fontFamily: "'Noto Sans TC','Noto Sans SC',sans-serif" }}>
            {isRunning ? "禪修中…" : "準備就緒"}
          </span>
        </div>
      </div>
      ...
      {timeLeft === 0 && (
        <div className="fade-in" style={{
          ...
          fontFamily: "'Noto Serif TC','Noto Serif SC',serif", fontSize: "1rem", color: "#7a4c10",
        }}>
          🙏 禪修圓滿，阿彌陀佛
        </div>
      )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem" }}>
            {[
              ...
              <div key={item.title} className="zen-card" style={{ padding: "1.25rem" }}>
                <h3 style={{ fontFamily: "'Noto Serif TC','Noto Serif SC',serif", fontSize: "1rem", color: "#2c1810", marginBottom: "0.5rem" }}>
                  {item.title}
                </h3>
                <p style={{ fontFamily: "'Noto Sans TC','Noto Sans SC',sans-serif", fontSize: "0.9rem", color: "#8a5a2f", marginBottom: "0.75rem" }}>
                  {item.desc}
                </p>
                <AudioPlayer
                  text={item.text} label="聆聽" size="sm" isStatic={true}
                  audioUrl={item.url} trackEvent={events.LISTEN_COURSE}
        ...
        <section>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {dailySutras.map((sutra, idx) => (
              <div key={sutra.id} className="zen-card" style={{
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
                  <p style={{ fontFamily: "'Noto Serif TC','Noto Serif SC',serif", fontSize: "1rem", color: "#2c1810", lineHeight: 1.8, margin: 0, flex: 1 }}>
                    「{sutra.text}」
                  </p>
                ...
                {selectedIdx === idx && (
                  <div className="fade-in" style={{ marginTop: "1rem" }}>
                    <p style={{ fontSize: "0.9rem", color: "#a06810", fontFamily: "'Noto Sans TC','Noto Sans SC',sans-serif", marginBottom: "0.75rem" }}>
                      —— {sutra.source}
                    </p>
                    <p style={{ fontFamily: "'Noto Sans TC','Noto Sans SC',sans-serif", fontSize: "0.95rem", lineHeight: 1.8, color: "#5c3d2e", marginBottom: "0.75rem" }}>
                      💡 {sutra.explanation}
                    </p>
                    <AudioPlayer
                      text={`${sutra.text}。摘自${sutra.source}。${sutra.explanation}`}
                      label="朗讀" size="sm" isStatic={true}
                      audioUrl={quoteAudios[idx] ?? null}
                      trackEvent={events.LISTEN_QUOTE} trackProps={{ sutraId: sutra.id }}
                    />
                  </div>
                )}
              </div>
